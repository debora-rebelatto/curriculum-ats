...
for (const file of files) {
  ...
  if (match) {
    ...
    fs.writeFileSync(path.join(folderPath, compName + '.component.html'), htmlContent, 'utf8');
    
    let newTsContent = content.replace(/template:\s*`[\s\S]*?`/, `templateUrl: './${compName}.component.html'`);
...
appContent = appContent.replace(/from '\.\/components\/([a-z\-]+)\.component'/g, "from './components/$1/$1.component'");
